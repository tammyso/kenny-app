...
export default function EditInvoiceForm({
  ...
  const [eventLocation, setEventLocation] = useState(invoice.event_location ?? "");
  const [eventTime, setEventTime] = useState(invoice.event_time ?? "");
  const [lineItems, setLineItems] = useState<LineItemUI[]>(
    invoice.line_items.length > 0
      ? invoice.line_items.map((item) => ({ ...item, id: newId() }))
      : [{ id: newId(), description: "", quantity: 1, unitPrice: 0 }],
  );
  const [selectedPreset, setSelectedPreset] = useState("");
  const [discountDollars, setDiscountDollars] = useState(invoice.discount_amount / 100);
  const [paymentType, setPaymentType] = useState<"retainer" | "full">(
    invoice.payment_type === "full" ? "full" : "retainer",
  );
  const [retainerDueDate, setRetainerDueDate] = useState(invoice.retainer_due_date ?? "");
  ...
  const handleEventDateChange = (val: string) => {
    setEventDate(val);
    if (val) setBalanceDueDate(addDays(val, -14));
  };
  ...
  const validate = () => {
    ...
    if (lineItems.filter((i) => i.description.trim()).length === 0) return "Add at least one line item.";
    ...
    startTransition(async () => {
      const result = await updateInvoice(invoiceId, {
        ...
        lineItems: lineItems
          ...
          .map(({ description, quantity, unitPrice }) => ({ description, quantity, unitPrice })),
        discountDollars,
        paymentType,
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="rounded-xl border border-zinc-200 bg-white p-6">
        <h2 className="mb-4 text-sm font-semibold text-zinc-700">Event Details</h2>
        <div className="grid gap-4 sm:grid-cols-3">
          <div>
            <label className={labelClass}>Event Date</label>
            <input type="date" value={eventDate} onChange={(e) => handleEventDateChange(e.target.value)} style={{ colorScheme: "light" }} className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>Time</label>
            <input type="text" value={eventTime} onChange={(e) => setEventTime(e.target.value)} className={inputClass} placeholder="5:30pm – 8:30pm" />
          </div>
          <div>
            <label className={labelClass}>Location</label>
            <input type="text" value={eventLocation} onChange={(e) => setEventLocation(e.target.value)} className={inputClass} placeholder="14 East Main St" />
          </div>
        </div>
      </section>
      ...
      <section className="rounded-xl border border-zinc-200 bg-white p-6">
            ...
            {PRESET_GROUPS.map((group) => (
              <optgroup key={group} label={group}>
                {PRESETS.filter((p) => p.group === group).map((p) => (
                  <option key={p.label} value={p.label}>{p.label} — {fmt(p.price)}</option>
                ))}
              </optgroup>
            ))}
          </select>
          <button type="button" onClick={addPreset} disabled={!selectedPreset} className="rounded-lg border border-zinc-200 px-4 py-2 text-sm font-medium text-zinc-700 transition hover:bg-zinc-50 disabled:cursor-not-allowed disabled:opacity-40">Add</button>
        </div>
              ...
              {lineItems.map((item) => (
                <tr key={item.id}>
                  <td className="py-2 pr-3">
                    <input type="text" value={item.description} onChange={(e) => updateItem(item.id, "description", e.target.value)} className={inputClass} placeholder="Service description" />
                  </td>
                  <td className="py-2 pr-3">
                    <input type="number" min="1" step="1" value={item.quantity} onChange={(e) => updateItem(item.id, "quantity", e.target.value)} className={inputClass} />
                  </td>
                  <td className="py-2 pr-3">
                    <div className="relative">
                      <span className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400">$</span>
                      <input type="number" min="0" step="0.01" value={item.unitPrice} onChange={(e) => updateItem(item.id, "unitPrice", e.target.value)} className={`${inputClass} pl-6`} />
                    </div>
                  </td>
                  <td className="py-2 pr-3 text-right font-medium text-zinc-700">{fmt(item.quantity * item.unitPrice)}</td>
                  <td className="py-2">
                    <button type="button" onClick={() => removeItem(item.id)} className="rounded p-1 text-zinc-300 hover:bg-zinc-100 hover:text-zinc-600" aria-label="Remove">×</button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <button type="button" onClick={addCustomRow} className="text-xs font-medium text-zinc-400 underline-offset-2 hover:text-zinc-700 hover:underline">+ Add custom line</button>
      </section>
      ...
      <section className="rounded-xl border border-zinc-200 bg-white p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400">$</span>
              <input type="number" min="0" step="0.01" value={discountDollars} onChange={(e) => setDiscountDollars(parseFloat(e.target.value) || 0)} className={`${inputClass} pl-6`} />
            </div>
          </div>
          <div>
            ...
            <div className="flex gap-2">
              {(["retainer", "full"] as const).map((type) => (
                <button key={type} type="button" onClick={() => setPaymentType(type)} className={`flex-1 rounded-lg border px-3 py-2 text-sm font-medium transition ${paymentType === type ? "border-zinc-900 bg-zinc-900 text-white" : "border-zinc-200 text-zinc-600 hover:bg-zinc-50"}`}>
        ...
        {paymentType === "retainer" && (
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input type="date" value={retainerDueDate} onChange={(e) => setRetainerDueDate(e.target.value)} style={{ colorScheme: "light" }} className={inputClass} />
            </div>
            <div>
              <label className={labelClass}>
                Balance due date
                {eventDate && <span className="ml-1 text-zinc-400">(auto-set 14 days before event)</span>}
              </label>
              <input type="date" value={balanceDueDate} onChange={(e) => setBalanceDueDate(e.target.value)} style={{ colorScheme: "light" }} className={inputClass} />
            </div>
          </div>
        )}
        <div className="mt-6 space-y-2 border-t border-zinc-100 pt-4 text-sm">
          ...
          {discountDollars > 0 && <div className="flex justify-between text-zinc-500"><span>Discount</span><span>−{fmt(discountDollars)}</span></div>}
          <div className="flex justify-between font-semibold text-zinc-900"><span>Total</span><span>{fmt(total)}</span></div>
          {paymentType === "retainer" && total > 0 && (
            <>
              <div className="flex justify-between text-zinc-500"><span>30% Retainer due now</span><span>{fmt(retainerAmount)}</span></div>
              <div className="flex justify-between text-zinc-500"><span>Remaining balance</span><span>{fmt(balanceAmount)}</span></div>
            </>
          )}
        </div>
      </section>
      ...
      <div className="flex gap-3">
        <button type="button" onClick={() => router.back()} className="inline-flex h-10 items-center rounded-lg border border-zinc-200 px-5 text-sm font-medium text-zinc-500 transition hover:bg-zinc-50">
          Cancel
        </button>
        <button type="button" onClick={handleSave} disabled={isPending} className="inline-flex h-10 items-center rounded-lg bg-zinc-900 px-5 text-sm font-medium text-white transition hover:bg-zinc-700 disabled:opacity-50">
